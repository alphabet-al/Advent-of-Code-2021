...
# the keys if you can boost its signal strength high enough; there's a little meter that indicates
...
def l_counter(data):
    ...
    return (icounter, dcounter)

def three_measure_data(data):
    tmd = []

    for i in range(len(data)):
        try:
            tmd.append(data[i] + data[i+1] + data[i+2])
        except:
            break
...
if __name__ == "__main__":
    ...
    # print(data)
    data = three_measure_data(data)
    # print(data)
    icounter, dcounter = l_counter(data)

    print ('{0} measurements larger than previous value'.format(icounter))
    print ('{0} measurements smaller than previous value'.format(dcounter))
